...
class UNet_plus_plus:
  """
  Class for Deep Learning Hyperspectral Segmentation with the one similar to the 2018 UNet++ by Zhou et.al.
  ...
  def train(self):
    ...
    if self.utils.continue_training == True:
        ...
        unet = load_model('saved_models/unet_plus_plus_best_model.h5', custom_objects=custom_objects)
        unet.summary()
  ...
  def predict(self, new_data = None):
    ...
    '''
    unet = self.build_3d_unet_plus_plus()
    unet.load_weights('saved_models/unet_plus_plus_best_model.h5')

...
from tensorflow.keras.layers import Input, Conv3D, Reshape, concatenate, Conv2D, MaxPooling3D, BatchNormalization, Add, Activation, Dropout, UpSampling2D, Flatten, GlobalMaxPooling2D, Layer, InputSpec, UpSampling3D, Conv3DTranspose
...
class UNet3Plus:
  ...
  def train(self):
    ...
    if self.utils.continue_training == True:
        ...
        custom_objects = {'unet3p_hybrid_loss': self.unet3p_hybrid_loss}
        # Load the full model, including optimizer state
        unet = load_model('saved_models/unet_3plus_best_model.h5', custom_objects=custom_objects)
        unet.summary()
  ...
  def predict(self, new_data = None):
    ...
    '''
    unet = self.build_3d_unet_3plus()
    unet.load_weights('saved_models/unet_3plus_best_model.h5')
    if new_data is not None:
      ...
      prediction_result = unet.predict(X_test)[0]
      ...
      for i in range(0, total_test_length, self.utils.batch_size):
        ...
        prediction_result[i:i+self.utils.batch_size] = unet.predict(self.utils.X_test[i:i+self.utils.batch_size])[0]

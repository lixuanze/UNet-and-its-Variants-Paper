...
class Residual_Attention_UNet3Plus:
  """
  ...
  def encoding_layers_building_blocks(self, units, in_layer, downsampling=True, is_bn = True, is_activation = True):
      '''
      ...
      if downsampling:
          ...
          if is_activation:
              convolution_layer = Activation('relu')(convolution_layer)
          output_layer = Conv3D(units, (3, 3, 3), padding='same', kernel_initializer='he_normal')(convolution_layer)
          output_layer = Add()([output_layer, shortcut]) # Residual Connections
          return output_layer
  ...
  def focal_loss(self, y_true, y_pred):
    ...
    weight = tf.multiply(y_true_c, tf.pow(tf.subtract(1., model_out), gamma))
    
    fl = tf.multiply(alpha, tf.multiply(weight, ce))
    reduced_fl = tf.reduce_sum(fl, axis=-1)  # Sum the focal loss over classes for each sample
    return tf.reduce_mean(reduced_fl)  # Mean over the batch

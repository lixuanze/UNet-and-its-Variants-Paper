...
class Residual_UNet:
  ...
  def encoding_layers_building_blocks(self, units, in_layer, downsampling=True):
      ...
      '''
      if downsampling:
          shortcut = Conv3D(units, (1, 1, 1), strides=(2, 2, 2), padding='same', kernel_initializer='he_normal')(in_layer)
          convolution_layer = BatchNormalization()(in_layer)
          convolution_layer = Activation('relu')(convolution_layer)
          convolution_layer = Conv3D(units, (3, 3, 3), strides=(2, 2, 2), padding='same', kernel_initializer='he_normal')(convolution_layer)
      else:
  ...
  def decoding_layers_building_blocks(self, units, in_layer, concat_layer):
      ...
      if concat_layer.shape[3] == 1:
          upsampling_layer = concatenate([in_layer, concat_layer], axis=4)
          upsampling_layer = UpSampling3D(size=(2, 2, 1))(upsampling_layer)
      else:
          in_layer = UpSampling3D(size=(1, 1, 2))(in_layer)
          upsampling_layer = concatenate([in_layer, concat_layer], axis=4)
          upsampling_layer = UpSampling3D(size=(2, 2, 2))(upsampling_layer)
  ...
  def build_3d_residual_unet(self):
    ...
    shortcut_1 = Conv3D(64, (1, 1, 1), strides=(2, 2, 2), padding='same', kernel_initializer='he_normal')(input_layer)
    down_sampling_convolution_layer_1 = Conv3D(64, (3, 3, 3), strides=(2, 2, 2), padding='same', kernel_initializer='he_normal')(input_layer)
    down_sampling_convolution_layer_1 = BatchNormalization()(down_sampling_convolution_layer_1)
    down_sampling_convolution_layer_1 = Activation('relu')(down_sampling_convolution_layer_1)
    down_sampling_convolution_layer_1 = Conv3D(64, (3, 3, 3), padding='same', kernel_initializer='he_normal')(down_sampling_convolution_layer_1)
    down_sampling_output_layer_1 = Add()([down_sampling_convolution_layer_1, shortcut_1]) # Residual Connections

    down_sampling_output_layer_2 = self.encoding_layers_building_blocks(128, down_sampling_output_layer_1)
    down_sampling_output_layer_3 = self.encoding_layers_building_blocks(256, down_sampling_output_layer_2)
    down_sampling_output_layer_4 = self.encoding_layers_building_blocks(512, down_sampling_output_layer_3)
    # encoding blocks
    encoding_space_output_layer = self.encoding_layers_building_blocks(1024, down_sampling_output_layer_4, downsampling=False)
    # up sampling blocks
    up_sampling_output_layer_1 = self.decoding_layers_building_blocks(512, encoding_space_output_layer, down_sampling_output_layer_4)
    up_sampling_output_layer_2 = self.decoding_layers_building_blocks(256, up_sampling_output_layer_1, down_sampling_output_layer_3)
    up_sampling_output_layer_3 = self.decoding_layers_building_blocks(128, up_sampling_output_layer_2, down_sampling_output_layer_2)
    up_sampling_output_layer_4 = self.decoding_layers_building_blocks(64, up_sampling_output_layer_3, down_sampling_output_layer_1)
  ...
  def train(self):
    ...
    if self.utils.continue_training == True:
        ...
        if self.utils.override_trained_optimizer:
            learning_rate_scheduler = tf.keras.optimizers.schedules.ExponentialDecay(initial_learning_rate=1e-3, decay_steps=20000, decay_rate=0.99)
            residual_unet.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = learning_rate_scheduler), loss={'output_layer': 'categorical_crossentropy'}, metrics=[tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features)])
    else:
  ...
  def predict(self, new_data = None):
    ...
    if new_data is not None:
      ...
      prediction_result = residual_unet.predict(X_test)
      ...
      for i in range(0, total_test_length, self.utils.batch_size):
        ...
        prediction_result[i:i+self.utils.batch_size] = residual_unet.predict(self.utils.X_test[i:i+self.utils.batch_size])

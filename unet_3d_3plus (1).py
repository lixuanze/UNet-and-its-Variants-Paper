...
class UNet3Plus:
  ...
  def build_3d_unet_3plus(self):
    ...
    if e4_d4.shape[3] == 1:
        e5_d4 = UpSampling3D(size=(2, 2, 1))(encoding_space_output_layer)
    else:
    ...
    if e3_d3.shape[3] == 1:
        e4_d3 = UpSampling3D(size=(2, 2, 1))(d4)
    else:
        e4_d3 = UpSampling3D(size=(2, 2, 2))(d4)
    e4_d3 = self.decoding_layers_building_blocks(cat_channels, e4_d3)

    if e3_d3.shape[3] == 1:
        e5_d3 = UpSampling3D(size=(4, 4, 1))(encoding_space_output_layer)
    else:
        if down_sampling_convolution_layer_4.shape[3] == 1:
            e5_d3 = UpSampling3D(size=(4, 4, 2))(encoding_space_output_layer)
        else:
    ...
    if e2_d2.shape[3] == 1:
        e3_d2 = UpSampling3D(size=(2, 2, 1))(d3)
    else:
        e3_d2 = UpSampling3D(size=(2, 2, 2))(d3)
    e3_d2 = self.decoding_layers_building_blocks(cat_channels, e3_d2)

    if e2_d2.shape[3] == 1:
        e4_d2 = UpSampling3D(size=(4, 4, 1))(d4)
    else:
        if down_sampling_convolution_layer_3.shape[3] == 1:
            e4_d2 = UpSampling3D(size=(4, 4, 2))(d4)
        else:
            e4_d2 = UpSampling3D(size=(4, 4, 4))(d4)
    e4_d2 = self.decoding_layers_building_blocks(cat_channels, e4_d2)
    
    if e2_d2.shape[3] == 1:
        e5_d2 = UpSampling3D(size=(8, 8, 1))(encoding_space_output_layer)
    else:
        if down_sampling_convolution_layer_3.shape[3] == 1:
            e5_d2 = UpSampling3D(size=(8, 8, 2))(encoding_space_output_layer)
        else:
            if down_sampling_convolution_layer_4.shape[3] == 1:
                e5_d2 = UpSampling3D(size=(8, 8, 4))(encoding_space_output_layer)
            else:
    ...
    if e1_d1.shape[3] == 1:
        e2_d1 = UpSampling3D(size=(2, 2, 1))(d2)
    else:
        e2_d1 = UpSampling3D(size=(2, 2, 2))(d2)
    e2_d1 = self.decoding_layers_building_blocks(cat_channels, e2_d1)

    if e1_d1.shape[3] == 1:
        e3_d1 = UpSampling3D(size=(4, 4, 1))(d3)
    else:
        if down_sampling_convolution_layer_2.shape[3] == 1:
            e3_d1 = UpSampling3D(size=(4, 4, 2))(d3)
        else:
            e3_d1 = UpSampling3D(size=(4, 4, 4))(d3)
    e3_d1 = self.decoding_layers_building_blocks(cat_channels, e3_d1)

    if e1_d1.shape[3] == 1:
        e4_d1 = UpSampling3D(size=(8, 8, 1))(d4)
    else:
        if down_sampling_convolution_layer_2.shape[3] == 1:
            e4_d1 = UpSampling3D(size=(8, 8, 2))(d4)
        else:
            if down_sampling_convolution_layer_3.shape[3] == 1:
                e4_d1 = UpSampling3D(size=(8, 8, 4))(d4)
            else:
                e4_d1 = UpSampling3D(size=(8, 8, 8))(d4)
    e4_d1 = self.decoding_layers_building_blocks(cat_channels, e4_d1)

    if e1_d1.shape[3] == 1:
        e5_d1 = UpSampling3D(size=(16, 16, 1))(encoding_space_output_layer)
    else:
        if down_sampling_convolution_layer_2.shape[3] == 1:
            e5_d1 = UpSampling3D(size=(16, 16, 2))(encoding_space_output_layer)
        else:
            if down_sampling_convolution_layer_3.shape[3] == 1:
                e5_d1 = UpSampling3D(size=(16, 16, 4))(encoding_space_output_layer)
            else:
                if down_sampling_convolution_layer_4.shape[3] == 1:
                    e5_d1 = UpSampling3D(size=(16, 16, 8))(encoding_space_output_layer)
                else:
    ...
    if self.utils.continue_training == True:
        ...
        if self.utils.override_trained_optimizer:
            ...
            if self.utils.deep_supervision == True:
                unet.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = learning_rate_scheduler), loss={'output_layer_1': self.unet3p_hybrid_loss, 'output_layer_2': self.unet3p_hybrid_loss, 'output_layer_3': self.unet3p_hybrid_loss, 'output_layer_4': self.unet3p_hybrid_loss, 'output_layer_5': self.unet3p_hybrid_loss}, metrics={'output_layer_1': tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features), 'output_layer_2': tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features), 'output_layer_3': tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features), 'output_layer_4': tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features), 'output_layer_5': tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features)})
            else:
                unet.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = learning_rate_scheduler), loss={'output_layer_1': self.unet3p_hybrid_loss}, metrics={'output_layer_1': tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features)})
    else:

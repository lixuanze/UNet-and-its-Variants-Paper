...
from tensorflow.keras.layers import Input, Conv3D, Reshape, concatenate, Conv2D, MaxPooling3D, UpSampling3D, BatchNormalization, Add, Activation, Dropout, Flatten, Lambda, Multiply
...
class Attention_UNet_plus_plus:
  ...
  def train(self):
    ...
    if self.utils.continue_training == True:
        ...
        attention_unet = load_model('models/attention_unet_plus_plus_best_model.h5', custom_objects=custom_objects)
  ...
  def predict(self, new_data = None):
    ...
    '''
    attention_unet = self.build_3d_attention_unet_plus_plus()
    attention_unet.load_weights('models/attention_unet_plus_plus_best_model.h5')

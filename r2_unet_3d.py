...
class R2UNet:
  ...
  def encoding_layers_building_blocks(self, units, in_layer, pooling_layer=True):
      ...
      for i in range(self.utils.r2unet_stacks):
        internal_residual_layer = Conv3D(units, (3, 3, 3), padding='same', kernel_initializer='he_normal')(convolution_layer_main)
        internal_residual_layer = BatchNormalization()(internal_residual_layer)
        ...
        for j in range(self.utils.r2unet_recur_num):
            ...
            internal_residual_layer = Conv3D(units, (3, 3, 3), padding='same', kernel_initializer='he_normal')(add_layer)
            internal_residual_layer = BatchNormalization()(internal_residual_layer)
            internal_residual_layer = Activation('relu')(internal_residual_layer)
  ...
  def build_3d_r2unet(self):
    ...
    model.compile(optimizer = tf.keras.optimizers.Adam(learning_rate = learning_rate_scheduler), loss={'output_layer': self.CCE_dice_loss}, metrics=[tf.keras.metrics.MeanIoU(num_classes = self.utils.n_features)])
    return model
